...
void FlyState_update(void)
{
	switch(FlyState)
	{
		case WATTING:
			...
			{
				act_control.group_mix = 2;
				act_control.controls[0] = 0.5;
				send_pwm_pub.publish(act_control);
...
int main(int argc, char **argv)
{
	...
	send_pwm_pub = nh.advertise<mavros_msgs::ActuatorControl>("/mavros/actuator_control",10);

	cout << "offb_servo node started!!!" << endl; 
